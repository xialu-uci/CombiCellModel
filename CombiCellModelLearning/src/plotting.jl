...
function compute_metrics_per_ligand_condition(dataTrue, fitData, savedir; o1_only = false)
    ...
    o1_max_00 = maximum(dataTrue["O1_00"])
    o2_max_00 = maximum(dataTrue["O2_00"])
...
function create_metrics_heatmaps(base_path::String; namingConv = nothing, rmse_normed_ref::Float64 = nothing)
    """
    ...
    max_abs_cd2 = isempty(valid_log_cd2) ? 1.0 : maximum(abs, valid_log_cd2)

    hm_cd2 = heatmap!(ax_cd2, 1:12, 1:12, log_cd2,
                    colormap=:RdBu,
                    colorrange=(-max_abs_cd2, max_abs_cd2),  # symmetric → 0 = white
                    nan_color=:lightgray)
    ...
    max_abs_pd1 = isempty(valid_log_pd1) ? 1.0 : maximum(abs, valid_log_pd1)

    hm_pd1 = heatmap!(ax_pd1, 1:12, 1:12, log10.(pd1_ratio_matrix),
                      colormap=:RdBu,
                      colorrange=(-max_abs_pd1, max_abs_pd1),
                      nan_color=:lightgray)
